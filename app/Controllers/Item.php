...
class Item extends BaseController {
    ...
    protected $pemasok;
    private $rules = [
        'barcode'  => ['rules' => 'required|alpha_numeric_punct|is_unique[tb_menu.barcode,id,{id}]'],
        'nama'     => ['rules' => 'required|alpha_numeric_punct'],
        'harga'    => ['rules' => 'required|numeric'],
        'stok'     => ['rules' => 'required|numeric'],
        'foto'   => ['rules' => 'max_size[gambar,2048]|mime_in[gambar,image/png,image/jpg,image/jpeg]|ext_in[gambar,png,jpg,jpeg]|is_image[gambar]'],
    ];
    ...
    public function ajax() {
        if ($this->request->isAJAX()) {
            return DataTables::use ('menu')
                ->select('menu.id AS iditem, barcode, nama, harga, stok, foto')
                ->make();
        }
    }
    ...
    public function detail() {
        ...
        $nama = $this->request->getGet('nama', FILTER_SANITIZE_SPECIAL_CHARS);
        $data    = $this->itemModel->detailnama($nama);
        if (!empty($data)) {
            return $this->response->setJSON($data);
        }
    }
        ...
        if ($this->request->isAJAX()) {
            if (!$this->validate($this->rules)) {
                ...
                $data = [
                    ...
                    'nama'   => ucwords($this->request->getPost('nama', FILTER_SANITIZE_SPECIAL_CHARS)),
